...
// FIX 1: Use pip_size from tick to correctly extract last digit
...
const scoreDigit = (digit, freq, total, streaks, windowFreq, windowTotal) => {
  ...
  const streak = streaks[digit];

  let matchScore = 0;
  if (globalPct > 10) matchScore += (globalPct - 10) * 2;
  if (windowPct > 10) matchScore += (windowPct - 10) * 3;
  if (streak <= 3) matchScore += 5;

  let differsScore = 0;
  if (globalPct < 10) differsScore += (10 - globalPct) * 2;
  if (windowPct < 10) differsScore += (10 - windowPct) * 3;
  if (streak >= 15) differsScore += Math.min((streak - 10) * 2, 20);
...
function useDerivStream(symbol, enabled) {
  ...
  const [pipSize, setPipSize] = useState(2); // FIX 2: track pip_size per symbol
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (msg.tick) {
            ...
            // FIX 4: Read pip_size directly from each tick message
            const ps = typeof msg.tick.pip_size === "number" ? msg.tick.pip_size : 2;
            setPipSize(ps);
            setLastPrice({ value: price, pipSize: ps });
            const digit = getLastDigit(price, ps);
            setHistory(prev => {
      ...
      ws.onerror = (e) => {
        ...
        setConnected(false);
        setError("Connection error — retrying in 3s...");
      };

      ws.onclose = () => {
        setConnected(false);
        // FIX 5: Auto-reconnect on unexpected close
        if (!cancelled) {
  ...
  return {
    ...
    connected,
    error,
    reset: () => { setHistory([]); setLastPrice(null); }
  };
}
        ...
        return (
          <div key={d} style={{
            ...
            <div style={{
              ...
              color: isMatch ? "#a78bfa" : isDiff ? "#f472b6" : "#64748b", lineHeight: 1
            }}>{d}</div>
...
export default function MatchesDiffersPredictor() {
  ...
  const [windowSize, setWindowSize] = useState(50); // FIX 6: renamed from 'window' (reserved word)
  const [manualInput, setManualInput] = useState("");
  const [useManual, setUseManual] = useState(false);
  const [manualHistory, setManualHistory] = useState([]);

  const { history: liveHistory, lastPrice, pipSize, connected, error, reset } = useDerivStream(symbol, streaming && !useManual);
  ...
  // FIX 7: Display price correctly using actual pip_size
  const displayPrice = lastPrice
    ? lastPrice.value.toFixed(lastPrice.pipSize)
    : null;

  return (
    <div style={{
      minHeight: "100vh", background: "#06030f", color: "#e2e8f0",
      fontFamily: "'Inter',system-ui,sans-serif", padding: "20px 16px 40px",
    }}>
      <div style={{ maxWidth: "760px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "22px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
            <div style={{
              ...
              boxShadow: connected && !useManual ? "0 0 10px #22c55e" : "none", flexShrink: 0
            }} />
            ...
            <span style={{ marginLeft: "auto", fontSize: "10px", color: total >= 50 ? "#22c55e" : "#f59e0b", fontFamily: "monospace" }}>
              {total} ticks {total < 50 ? `(need ${50 - total} more)` : "✓ Ready"}
            </span>
          </div>
          <h1 style={{ margin: 0, fontSize: "20px", fontWeight: "700", letterSpacing: "-0.02em", color: "#e2e8f0" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: "10px", marginBottom: "12px", alignItems: "end" }}>
          ...
          <div style={{ display: "flex", gap: "6px" }}>
            ...
            <button onClick={handleReset} style={{
              ...
              cursor: "pointer", border: "1px solid #1e1b2e", background: "transparent", color: "#475569"
            }}>↺</button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "4px", marginBottom: "16px" }}>
          {["Live Stream", "Manual Entry"].map((label, i) => (
            <button key={label} onClick={() => setUseManual(i === 1)}
              style={{
        ...
        {/* manual entry */}
        {useManual && (
          <div style={{ background: "#0d0a1a", border: "1px solid #1e1b2e", borderRadius: "8px", padding: "14px", marginBottom: "14px" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              <input value={manualInput} onChange={e => setManualInput(e.target.value)}
                ...
                style={{ flex: 1, background: "#06030f", border: "1px solid #1e1b2e", color: "#e2e8f0", borderRadius: "6px", padding: "8px 10px", fontSize: "13px", fontFamily: "monospace" }} />
              <button onClick={handleManualAdd} style={{ padding: "8px 16px", borderRadius: "6px", fontSize: "12px", fontWeight: "600", cursor: "pointer", border: "none", background: "#7c3aed", color: "#fff" }}>Add</button>
            </div>
          </div>
        )}
        ...
        {total === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 20px", color: "#1e1b2e", fontSize: "13px" }}>
          ...
          <>
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "12px", paddingBottom: "8px", borderBottom: "1px solid #1e1b2e" }}>
                <div style={{ background: "#a78bfa20", border: "1px solid #a78bfa40", borderRadius: "4px", padding: "3px 10px", fontSize: "11px", fontWeight: "700", color: "#a78bfa", letterSpacing: "0.1em" }}>MATCHES</div>
                <span style={{ fontSize: "11px", color: "#475569" }}>Trade: last digit = your pick</span>
                <span style={{ marginLeft: "auto", fontSize: "10px", color: "#334155" }}>Top 3 picks by signal</span>
              </div>
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "12px", paddingBottom: "8px", borderBottom: "1px solid #1e1b2e" }}>
                <div style={{ background: "#f472b620", border: "1px solid #f472b640", borderRadius: "4px", padding: "3px 10px", fontSize: "11px", fontWeight: "700", color: "#f472b6", letterSpacing: "0.1em" }}>DIFFERS</div>
                <span style={{ fontSize: "11px", color: "#475569" }}>Trade: last digit ≠ your pick</span>
                <span style={{ marginLeft: "auto", fontSize: "10px", color: "#334155" }}>Avoid these digits</span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "10px" }}>
                {top3Differs.map((s, i) => (
            ...
            {/* Digit grid */}
            <div style={{ background: "#0a0714", border: "1px solid #1e1b2e", borderRadius: "10px", padding: "16px", marginBottom: "16px" }}>
              <div style={{ fontSize: "10px", color: "#475569", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: "12px", display: "flex", justifyContent: "space-between" }}>
                <span>All Digits · {windowSize}-tick window</span>
                <span>
                  <span style={{ color: "#a78bfa" }}>■ matches</span>{"  "}
                  <span style={{ color: "#f472b6" }}>■ differs</span>
                </span>
              </div>
              <DigitGrid freq={freq} total={total} windowFreq={windowFreq} windowTotal={windowTotal}
                streaks={streaks} matchPick={top3Match[0]?.digit} differsPick={top3Differs[0]?.digit} />
            </div>

            {/* Tick tape */}
            <div style={{ background: "#0a0714", border: "1px solid #1e1b2e", borderRadius: "10px", padding: "14px", marginBottom: "16px" }}>
              <div style={{ fontSize: "10px", color: "#334155", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: "10px" }}>
                Last 30 ticks
                {/* FIX 7: show price with correct decimal places */}
                {displayPrice && <span style={{ float: "right", fontFamily: "monospace", color: "#475569" }}>{displayPrice}</span>}
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "8px", marginBottom: "16px" }}>
              {[
                ...
                <div key={label} style={{ background: "#0a0714", border: "1px solid #1e1b2e", borderRadius: "8px", padding: "10px 12px", textAlign: "center" }}>
...
// ─── helpers ──────────────────────────────────────────────────────────────────
const getLastDigit = (price, pipSize = 2) => {
  const s = price.toFixed(pipSize);
